...
# test8.tst: test for MultiSeries[multiseries] (no. 8)
...
#test 
   ...
   #=============== test nb 8 =====================
   Order:=3;
   TRY[compare_SERIES]("8-1",
    op([assign(s=multiseries(op([foo(4), x]))),SERIES2Series(s)]),
    Series([foo(4)],0,algebraic,[0],infinity,integer,_var[x],foo(4)));
   unassign('s');
