...
# test2.tst: test for MultiSeries[multiseries] (no. 2)
...
#test 5
   ...
   #=============== test nb 2 =====================
   Order:=3;
   TRY[compare_SERIES]("2-1",
    op([assign(s=multiseries(op([1+x, x]))),SERIES2Series(s)]),
    Series([1, 1],0,integer,[0, 1],infinity,integer,_var[x],1+_var[x]));
   unassign('s');
