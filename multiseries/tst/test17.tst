...
# test17.tst: test for MultiSeries[multiseries] (no. 17)
...
#test 5
   ...
   #=============== test nb 17 =====================
   Order:=2;
   TRY[compare_SERIES]("17-1",
    op([assign(s=multiseries(op([1/(1+x)-1+x, x, 2, exact_order]))),
    SERIES2Series(s)]),
    Series([1, -1],1,integer,[2, 3],4,integer,_var[x],
    1/(1+_var[x])-1+_var[x]));
   unassign('s');
