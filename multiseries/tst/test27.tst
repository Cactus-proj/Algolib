...
# test27.tst: test for MultiSeries[multiseries] (no. 27)
...
#test 5
   ...
   Order:=3;
   assume(alpha,real);
   TRY[compare_SERIES]("27-1",
    op([assign(s=multiseries(op([x^alpha/(1-x), x]))),SERIES2Series(s)]),
    Series([1, 1, 1],1,integer,[alpha, 1+alpha, 2+alpha],3+alpha,algebraic,
    _var[x],1/(1-_var[x])*_var[x]^alpha));
