...
# test101.tst: test for MultiSeries[multiseries] (no. 101)
...
#test 5
   ...
   #=============== test nb 101 =====================
   Order:=3;
   TRY[compare_SERIES]("101-1",
    op([assign(s=multiseries(op([(sin(x)/x)^(sin(x)/x/(1-cos(x))), x]))),SERIES2Series(s)]),
    Series([exp(-1/3)],1,algebraic,[0],1,integer,_var[x],exp(-1/3)*exp(sin(_var[x])/
    _var[x]/(1-cos(_var[x]))*ln(sin(_var[x])/_var[x])+1/3)));
   unassign('s');
