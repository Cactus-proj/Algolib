...
# test103.tst: test for MultiSeries[multiseries] (no. 103)]
...
#test 5
   ...
   #=============== test nb 103 =====================
   Order:=3;
   TRY[compare_SERIES]("103-1",
    op([assign(s=multiseries(op([GAMMA(x), x]))),SERIES2Series(s)]),
    Series([1, -gamma, 1/12*Pi^2+1/2*gamma^2],1,algebraic,[-1, 0, 1],2,
    integer,_var[x],GAMMA(1+_var[x])/_var[x]));
   unassign('s');
