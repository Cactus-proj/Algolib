...
# test29.tst: test for MultiSeries[multiseries] (no. 29)
...
#test 5
   ...
   #=============== test nb 29 =====================
   Order:=3;
   TRY[compare_SERIES]("29-1",
    op([assign(s=multiseries(op([exp(1+x), x]))),SERIES2Series(s)]),
    Series([exp(1), exp(1), 1/2*exp(1)],1,algebraic,[0, 1, 2],3,integer,
    _var[x],exp(1)*exp(_var[x])));
   unassign('s');
