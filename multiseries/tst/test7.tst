...
# test7.tst: test for MultiSeries[multiseries] (no. 7)
...
#test 5
   ...
   #=============== test nb 7 =====================
   Order:=3;
   TRY[compare_SERIES]("7-1",
    op([assign(s=multiseries(op([x*(1+x)*(2+x)*(3+x)-6*x, x, 3, exact_order]))),
    SERIES2Series(s)]),Series([11, 6, 1],0,integer,[2, 3, 4],infinity,integer,
    _var[x],(1+_var[x])*_var[x]*(2+_var[x])*(3+_var[x])-6*
    _var[x]));
   unassign('s');
