...
# test99.tst: test for MultiSeries[multiseries] (no. 99)
...
#test 5
   ...
   #=============== test nb 99 =====================
   Order:=4;
   TRY[compare_SERIES]("99-1",
    op([assign(s=multiseries(op([1/(x^2-x^2*f(x)), x, 4]))),SERIES2Series(s)]),
    Series([1/(1-f(0)), 1/(1-f(0))^2*D(f)(0), 1/(1-f(0))*(1/2/(1-f(0))*
    `@@`(D,2)(f)(0)+1/(1-f(0))^2*D(f)(0)^2), 1/(1-f(0))*(1/6/(1-f(0))*
    `@@`(D,3)(f)(0)+1/(1-f(0))^2*`@@`(D,2)(f)(0)*D(f)(0)+1/(1-f(0))^3*D(f)(0)^3)],
    1,algebraic,[-2, -1, 0, 1],2,integer,_var[x],
    1/(_var[x]^2-f(_var[x])*_var[x]^2)));
   unassign('s');
