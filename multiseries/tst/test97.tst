...
# test97.tst: test for MultiSeries[multiseries] (no. 97)
...
#test 5
   ...
   #=============== test nb 97 =====================
   Order:=3;
   TRY[compare_SERIES]("97-1",
    op([assign(s=multiseries(op([f(x), x]))),SERIES2Series(s)]),
    Series([f(0), D(f)(0), 1/2*`@@`(D,2)(f)(0)],1,algebraic,[0, 1, 2],3,
    integer,_var[x],f(_var[x])));
   unassign('s');
