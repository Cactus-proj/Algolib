...
# test95.tst: test for MultiSeries[multiseries] (no. 95)
...
#test 5
   ...
   #=============== test nb 95 =====================
   Order:=3;
   TRY[compare_SERIES]("95-1",
    op([assign(s=eval(multiseries(op([foo(x)^x, x])),[Im(D(foo)(0))=0])),SERIES2Series(s)]),
    Series([1, ln(foo(0)), 1/foo(0)*D(foo)(0)+1/2*ln(foo(0))^2],1,algebraic,
    [0, 1, 2],3,integer,_var[x],exp((ln(1+(foo(_var[x])-foo(0))/
    foo(0))+ln(foo(0)))*_var[x])));
   unassign('s');
