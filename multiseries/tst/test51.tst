...
# test51.tst: test for MultiSeries[multiseries] (no. 51)
...
#test 6
   ...
   #=============== test nb 51 =====================
   Order:=3;
   TRY[compare_SERIES]("51-1",
    op([assign(s=multiseries(op([exp(1/x+exp(-1/x))-exp(1/x), x]))),
    SERIES2Series(s)]),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1)],
    Series([],1,rational,[],0,integer,_var[x],1/2),t_SERIES,[0],1,
    integer,_var[1/exp(1/x)],
    exp(_var[1/exp(1/x)])/_var[1/exp(1/x)]-1/_var[1/exp(1/x)]));
   TRY[compare_SERIES]("51-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([1, 1/2],1,rational,[0, 1],2,integer,_var[1/exp(1/x)],
    exp(_var[1/exp(1/x)])/_var[1/exp(1/x)]-1/_var[1/exp(1/x)]));
   TRY[compare_SERIES]("51-3",
    SERIES2Series(multiseries(op(9,s),op(1,s))),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1),
    Series([1/2],0,rational,[0],infinity,integer,_var[x],1/2)],
    Series([],1,integer,[],0,integer,_var[x],1),t_SERIES,[0, 1],2,
    integer,_var[1/exp(1/x)],
    exp(_var[1/exp(1/x)])/_var[1/exp(1/x)]-1/_var[1/exp(1/x)]));
   TRY[compare_SERIES]("51-4",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,exact_order)),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1),
    Series([1/2],0,rational,[0],infinity,integer,_var[x],1/2),
    Series([1/6],0,rational,[0],infinity,integer,_var[x],1/6)],
    Series([],1,integer,[],0,integer,_var[x],1/24),t_SERIES,
    [0, 1, 2],3,integer,_var[1/exp(1/x)],exp(_var[1/exp(1/x)])/
    _var[1/exp(1/x)]-1/_var[1/exp(1/x)]));
   unassign('s');
