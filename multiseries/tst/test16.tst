...
# test16.tst: test for MultiSeries[multiseries] (no. 16)
...
#test 5
   ...
   #=============== test nb 16 =====================
   Order:=2;
   TRY[compare_SERIES]("16-1",
    op([assign(s=multiseries(op([1/(1+x^3), x, 2, exact_order]))),
    SERIES2Series(s)]),
    Series([1, -1],1,integer,[0, 3],6,integer,_var[x],1/(1+_var[x]^3)));
   unassign('s');
