...
# test72.tst: test for MultiSeries[multiseries] (no. 72)
...
#test 5
   ...
   #=============== test nb 72 =====================
   Order:=4;
   TRY[compare_SERIES]("72-1",
    op([assign(s=multiseries(op([exp(I/x-x), x, 4]))),SERIES2Series(s)]),
    Series([exp(I/_var[x]), -exp(I/_var[x]), 1/2*exp(I/_var[x]),
    -1/6*exp(I/_var[x])],1/24*exp(I/_var[x]),algebraic,[0, 1, 2, 3],
    4,integer,_var[x],exp(I/_var[x])*exp(-_var[x])));
   unassign('s');
