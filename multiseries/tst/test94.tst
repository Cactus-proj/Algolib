...
# test94.tst: test for MultiSeries[multiseries] (no. 94)
...
#test 5
   ...
   #=============== test nb 94 =====================
   Order:=3;
   TRY[compare_SERIES]("94-1",
    op([assign(s=multiseries(op([foo(4)^x, x]))),SERIES2Series(s)]),
    Series([1, ln(foo(4)), 1/2*ln(foo(4))^2],1,algebraic,[0, 1, 2],3,
    integer,_var[x],exp(ln(foo(4))*_var[x])));
   unassign('s');
