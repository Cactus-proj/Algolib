...
# test20.tst: test for MultiSeries[multiseries] (no. 20)
...
#test 5
   ...
   #=============== test nb 20 =====================
   Order:=3;
   TRY[compare_SERIES]("20-1",
    op([assign(s=multiseries(op([1/(x^4+x^2)^3, x, 3, exact_order]))),
    SERIES2Series(s)]),
    Series([1, -3, 6],1,integer,[-6, -4, -2],0,integer,_var[x],
    1/(_var[x]^4+_var[x]^2)^3));
   unassign('s');
