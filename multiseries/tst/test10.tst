...
# test10.tst: test for MultiSeries[multiseries] (no. 10)
...
#test 5
   ...
   #=============== test nb 10 =====================
   Order:=4;
   TRY[compare_SERIES]("10-1",
    op([assign(s=multiseries(op([(x^2)^(1/2), x = 1, 4]))),SERIES2Series(s)]),
    Series([1, -1],1,integer,[0, 1],5,rational,_var[1-x],
    ((1-_var[1-x])^2)^(1/2)));
   unassign('s');
