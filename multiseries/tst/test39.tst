...
# test39.tst: test for MultiSeries[multiseries] (no. 39)
...
#test 5
   ...
   #=============== test nb 39 =====================
   Order:=3;
   TRY[compare_SERIES]("39-1",
    op([assign(s=multiseries(op([1/(1+x+exp(1/x)), x]))),SERIES2Series(s)]),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1)],
    Series([-1],1,algebraic,[0],1,integer,_var[x],-1-_var[x]),
    t_SERIES,[1],2,integer,_var[1/exp(1/x)],
    1/(1+_var[x]+1/_var[1/exp(1/x)])));
   TRY[compare_SERIES]("39-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([1, -1-_var[x], (1+_var[x])^2],1,algebraic,[1, 2, 3],4,
    integer,_var[1/exp(1/x)],1/(1+_var[x]+1/_var[1/exp(1/x)])));
   unassign('s');
