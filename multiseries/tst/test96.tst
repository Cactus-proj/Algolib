...
# test96.tst: test for MultiSeries[multiseries] (no. 96)
...
#test 5
   ...
   #=============== test nb 96 =====================
   Order:=3;
   TRY[compare_SERIES]("96-1",
    op([assign(s=multiseries(op([f(a+x+exp(-1/x)), x]))),SERIES2Series(s)]),
    Series([Series([f(a), D(f)(a), 1/2*`@@`(D,2)(f)(a)],1,algebraic,[0, 1, 2],
    3,integer,_var[x],f(a+_var[x]))],
    Series([],1,algebraic,[],0,integer,_var[x],D(f)(a+_var[x])),
    t_SERIES,[0],1,integer,_var[1/exp(1/x)],f(a+_var[x]+
    _var[1/exp(1/x)])));
   TRY[compare_SERIES]("96-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([f(a+_var[x]), D(f)(a+_var[x]),
    1/2*`@@`(D,2)(f)(a+_var[x])],1,algebraic,[0, 1, 2],3,integer,
    _var[1/exp(1/x)],f(a+_var[x]+_var[1/exp(1/x)])));
   unassign('s');
