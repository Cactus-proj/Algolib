...
# test25.tst: test for MultiSeries[multiseries] (no. 25)
...
#test 5
   ...
   #=============== test nb 25 =====================
   Order:=4;
   TRY[compare_SERIES]("25-1",
    op([assign(s=multiseries(op([1/(x^Pi+x^(2^(1/2))), x, 4]))),
    SERIES2Series(s)]),
    Series([1, -1, 1, -1],1,integer,
    [-2^(1/2), -2*2^(1/2)+Pi, -3*2^(1/2)+2*Pi, -4*2^(1/2)+3*Pi],-5*2^(1/2)+4*Pi,
    algebraic,_var[x],1/(_var[x]^Pi+_var[x]^(2^(1/2)))));
   unassign('s');
