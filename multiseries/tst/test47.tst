...
# test47.tst: test for MultiSeries[multiseries] (no. 47)
...
#test 8
   ...
   #=============== test nb 47 =====================
   Order:=4;
   TRY[compare_SERIES]("47-1",
    op([assign(s=multiseries(op([exp(x+exp(-1/x))-exp(x), x, 4]))),SERIES2Series(s)]),
    Series([Series([1, 1, 1/2, 1/6],1,rational,[0, 1, 2, 3],4,integer,_var[x],
    exp(_var[x]))],Series([1/2, 1/2, 1/4, 1/12],1,rational,[0, 1, 2, 3],4,
    integer,_var[x],1/2*exp(_var[x])),t_SERIES,[1],2,integer,
    _var[1/exp(1/x)],
    exp(_var[x])*exp(_var[1/exp(1/x)])-exp(_var[x])));
   TRY[compare_SERIES]("47-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([exp(_var[x]), 1/2*exp(_var[x]), 1/6*exp(_var[x])],
    1,algebraic,[1, 2, 3],4,integer,_var[1/exp(1/x)],
    exp(_var[x])*exp(_var[1/exp(1/x)])-exp(_var[x])));
   TRY[compare_SERIES]("47-3",
    ...
    4,integer,_var[1/exp(1/x)],
    exp(_var[x])*exp(_var[1/exp(1/x)])-exp(_var[x])));
   TRY[compare_SERIES]("47-4",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,exact_order)),
    Series([Series([1, 1, 1/2, 1/6],1,rational,[0, 1, 2, 3],4,
    ...
    t_SERIES,[1, 2, 3, 4],5,integer,_var[1/exp(1/x)],
    exp(_var[x])*exp(_var[1/exp(1/x)])-exp(_var[x])));
   unassign('s');
