...
# test6.tst: test for MultiSeries[multiseries] (no. 6)
...
#test 5
   ...
   #=============== test nb 6 =====================
   Order:=2;
   TRY[compare_SERIES]("6-1",
    op([assign(s=multiseries(op([x*(1+x)*(2+x)*(3+x), x, 2, exact_order]))),
    SERIES2Series(s)]),
    Series([6, 11],1,integer,[1, 2],3,integer,_var[x],(1+_var[x])*
    _var[x]*(2+_var[x])*(3+_var[x])));
   unassign('s');
