...
# test5.tst: test for MultiSeries[multiseries] (no. 5)
...
#test 5
   ...
   #=============== test nb 5 =====================
   Order:=2;
   TRY[compare_SERIES]("5-1",
    op([assign(s=multiseries(op([(1+x)*(2+x)*(3+x)-6-11*x, x, 2, exact_order]))),
    SERIES2Series(s)]),
    Series([6, 1],0,integer,[2, 3],infinity,integer,_var[x],(1+_var[x])*
    (2+_var[x])*(3+_var[x])-6-11*_var[x]));
   unassign('s');
