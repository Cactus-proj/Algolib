...
# test23.tst: test for MultiSeries[multiseries] (no. 23)
...
#test 5
   ...
   #=============== test nb 23 =====================
   Order:=4;
   TRY[compare_SERIES]("23-1",
    op([assign(s=multiseries(op([(x^Pi+x^(2^(1/2)))^4, x, 4]))),
    SERIES2Series(s)]),
    Series([1, 4, 6, 4],1,integer,[4*2^(1/2), 3*2^(1/2)+Pi, 2*2^(1/2)+2*Pi,
    2^(1/2)+3*Pi],4*Pi,algebraic,_var[x],
    (_var[x]^Pi+_var[x]^(2^(1/2)))^4));
   unassign('s');
