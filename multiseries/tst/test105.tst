...
# test105.tst: test for MultiSeries[multiseries] (no. 105)
...
#test 5
   ...
   #=============== test nb 105 =====================
   Order:=3;
   TRY[compare_SERIES]("105-1",
    op([assign(s=multiseries(op([GAMMA(x^Pi), x]))),SERIES2Series(s)]),
    Series([1, -gamma, 1/12*Pi^2+1/2*gamma^2],1,algebraic,[-Pi, 0, Pi],
    2*Pi,algebraic,_var[x],GAMMA(_var[x]^Pi+1)/(_var[x]^Pi)));
   unassign('s');
