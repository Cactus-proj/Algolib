...
# test15.tst: test for MultiSeries[multiseries] (no. 15)
...
#test 5
   ...
   #=============== test nb 15 =====================
   Order:=2;
   TRY[compare_SERIES]("15-1",
    op([assign(s=multiseries(op([1/(1+x), x, 2, exact_order]))),
    SERIES2Series(s)]),
    Series([1, -1],1,integer,[0, 1],2,integer,_var[x],1/(1+_var[x])));
   unassign('s');
