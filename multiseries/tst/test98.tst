...
# test98.tst: test for MultiSeries[multiseries] (no. 98)
...
#test 5
   ...
   #=============== test nb 98 =====================
   Order:=3;
   TRY[compare_SERIES]("98-1",
    op([assign(s=multiseries(op([f(1+x), x]))),SERIES2Series(s)]),
    Series([f(1), D(f)(1), 1/2*`@@`(D,2)(f)(1)],1,algebraic,[0, 1, 2],3,
    integer,_var[x],f(1+_var[x])));
   unassign('s');
