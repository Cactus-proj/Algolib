...
# test100.tst: test for MultiSeries[multiseries] (no. 100)
...
#test 5
   ...
   #=============== test nb 100 =====================
   Order:=4;
   TRY[compare_SERIES]("100-1",
    op([assign(s=multiseries(op([1/(x^2-x*sin(x)), x, 4]))),SERIES2Series(s)]),
    Series([6],1,integer,[-4],-2,integer,_var[x],
    1/(_var[x]^2-sin(_var[x])*_var[x])));
   unassign('s');
