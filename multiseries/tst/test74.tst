...
# test74.tst: test for MultiSeries[multiseries] (no. 74)
...
#test 5
   ...
   #=============== test nb 74 =====================
   Order:=3;
   TRY[compare_SERIES]("74-1",
    op([assign(s=multiseries(op([exp((-1+I)*x)+x, x = infinity]))),SERIES2Series(s)]),
    Series([Series([1],0,integer,[-1],infinity,integer,_var[1/x],
    1/_var[1/x])],Series([],1,algebraic,[],0,integer,_var[1/x],
    exp(I/_var[1/x])),t_SERIES,[0],1,integer,_var[1/exp(x)],
    exp(I/_var[1/x])*_var[1/exp(x)]+1/_var[1/x]));
   TRY[compare_SERIES]("74-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([1/_var[1/x], exp(I/_var[1/x])],0,algebraic,
    [0, 1],infinity,integer,_var[1/exp(x)],exp(I/_var[1/x])*
    _var[1/exp(x)]+1/_var[1/x]));
   TRY[compare_SERIES]("74-3",
    SERIES2Series(multiseries(op(9,s),op(1,s))),
    Series([Series([1],0,integer,[-1],infinity,integer,_var[1/x],
    1/_var[1/x]),
    Series([exp(I/_var[1/x])],0,algebraic,[0],infinity,integer,
    _var[1/x],exp(I/_var[1/x]))],0,t_SERIES,[0, 1],
    infinity,integer,_var[1/exp(x)],exp(I/_var[1/x])*
    _var[1/exp(x)]+1/_var[1/x]));
   TRY[compare_SERIES]("74-4",
