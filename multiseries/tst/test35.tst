...
# test35.tst: test for MultiSeries[multiseries] (no. 35)
...
#test 5
   ...
   #=============== test nb 35 =====================
   Order:=3;
   TRY[compare_SERIES]("35-1",
    op([assign(s=multiseries(op([exp(1/x)+x+exp(-1/x), x]))),SERIES2Series(s)]),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1)],
    Series([],1,algebraic,[],1,integer,_var[x],_var[x]),t_SERIES,
    [-1],0,integer,_var[1/exp(1/x)],
    1/_var[1/exp(1/x)]+_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("35-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([1, _var[x], 1],0,algebraic,[-1, 0, 1],infinity,integer,
    _var[1/exp(1/x)],1/_var[1/exp(1/x)]+_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("35-3",
    SERIES2Series(multiseries(op(9,s),op(1,s))),
    Series([Series([1],0,integer,[0],infinity,integer,_var[x],1),
    Series([1],0,integer,[1],infinity,integer,_var[x],_var[x]),
    Series([1],0,integer,[0],infinity,integer,_var[x],1)],0,t_SERIES,
    [-1, 0, 1],infinity,integer,_var[1/exp(1/x)],
    1/_var[1/exp(1/x)]+_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("35-4",
