...
# test33.tst: test for MultiSeries[multiseries] (no. 33)
...
#test 5
   ...
   #=============== test nb 33 =====================
   Order:=4;
   TRY[compare_SERIES]("33-1",
    op([assign(s=multiseries(op([1/x+exp(-1/x), x, 4]))),SERIES2Series(s)]),
    Series([Series([1],0,integer,[-1],infinity,integer,_var[x],1/_var[x])],
    Series([],1,integer,[],0,integer,_var[x],1),t_SERIES,[0],1,integer,
    _var[1/exp(1/x)],1/_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("33-2",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,op(1,s)[list][2..2])),
    Series([1/_var[x], 1],0,algebraic,[0, 1],infinity,integer,
    _var[1/exp(1/x)],1/_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("33-3",
    SERIES2Series(multiseries(op(9,s),op(1,s))),
    Series([Series([1],0,integer,[-1],infinity,integer,_var[x],1/_var[x]),
    Series([1],0,integer,[0],infinity,integer,_var[x],1)],0,t_SERIES,
    [0, 1],infinity,integer,_var[1/exp(1/x)],1/_var[x]+_var[1/exp(1/x)]));
   TRY[compare_SERIES]("33-4",
    SERIES2Series(multiseries(op(9,s),op(1,s),Order,exact_order)),
    Series([Series([1],0,integer,[-1],infinity,integer,_var[x],1/_var[x]),
    Series([1],0,integer,[0],infinity,integer,_var[x],1)],0,t_SERIES,[0, 1],
    infinity,integer,_var[1/exp(1/x)],1/_var[x]+_var[1/exp(1/x)]));
   unassign('s');
