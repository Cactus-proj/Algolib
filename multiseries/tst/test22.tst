...
# test22.tst: test for MultiSeries[multiseries] (no. 22)
...
#test 5
   ...
   #=============== test nb 22 =====================
   Order:=6;
   TRY[compare_SERIES]("22-1",
    member(op([assign(s=multiseries(op([x/(1+x)^2, x, 6]))),SERIES2Series(s)]),
[
    Series([1, -2, 3, -4, 5, -6],1,integer,[1, 2, 3, 4, 5, 6],7,integer,
    _var[x],1/(1+_var[x])^2*_var[x]),
    Series([1, -2, 3, -4, 5],1,integer,[1, 2, 3, 4, 5],6,integer,
    _var[x],1/(1+_var[x])^2*_var[x])
]), true
);
   unassign('s');
