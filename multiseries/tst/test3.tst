...
# test3.tst: test for MultiSeries[multiseries] (no. 3)
...
#test 5
   ...
   #=============== test nb 3 =====================
   Order:=1;
   TRY[compare_SERIES]("3-1",
    op([assign(s=multiseries(op([1+x, x, 1, exact_order]))),SERIES2Series(s)]),
    Series([1],1,integer,[0],1,integer,_var[x],1+_var[x]));
   unassign('s');
