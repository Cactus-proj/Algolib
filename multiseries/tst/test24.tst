...
# test24.tst: test for MultiSeries[multiseries] (no. 24)
...
#test 5
   ...
   #=============== test nb 24 =====================
   Order:=10;
   TRY[compare_SERIES]("24-1",
    op([assign(s=multiseries(op([1/(1-x-x^exp(1)), x, 10]))),
    SERIES2Series(s)]),
    Series([1, 1, 1, 1, 1, 2, 1, 3, 1, 1],1,integer,
    [0, 1, 2, exp(1), 3, exp(1)+1, 4, exp(1)+2, 5, 2*exp(1)],exp(1)+3,
    algebraic,_var[x],1/(1-_var[x]-_var[x]^exp(1))));
   unassign('s');
