...
local pt, ind, L, closedform, s, y, u, dominant, res, closeddom, genpt, final,i;
...
#	dominant:=DOMINANTTERM(pt,scale,var,res); # res is assigned
	...
	if res<>0 then
		...
		genpt=op(EXPR4SERIES,dominant),applyop(normal,LISTCOEFF,
			...
			procname('SERIES'(scale,[genpt,genpt],0,'algebraic',
			    [0,1],infinity,'integer',var[-1],genpt*(1+var[-1])),
			    args[2..-1]),
			'SERIES'(scale,[-'LambertW'(genpt)],0,'algebraic',
			    [0],infinity,'integer',var[-1],
			    -'LambertW'(genpt)))));
	...
	s:=series2SERIES(solve(
	    MultiSeries:-series((2+2*(y-1)*'exp'(y))^(1/2),y)=SCALEVARNAME,y),
	    args[2..-1]);
	ADDDOIT(-1,COMPOSE(
