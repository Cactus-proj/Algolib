...
###    -*-Maple-*-
###
###    Title:   `multiseries/function`[abs]
###    Created: Oct 2003
###    Author:  Bruno Salvy
...
`multiseries/function`[abs]:=proc(ser,scale,var,ord)::{SERIES,identical(0)};
local real,im;
option ALGOCOPYRIGHT; 
    ASSERT(ser::{'SERIES',identical(0),list({'SERIES',identical(0)})});
        
    if not type(ser,list) then # one argument case
	REALIMPART(ser,real,im) ;
	if im=0 then
	    if SIGN(real)=-1 then MULSERIESCST(ser,-1,integer)
	    else ser
	    fi
	else
	...
	if ADDDOIT(ser[1],-1)=0 then FUNCTIONTABLE[signum](ser[2],args[2..-1])
	else 0
	fi
    fi
end proc: # `multiseries/function`[abs]
