...
local res, fcn, largs, procfcn, vname, i, difforder, newexpr;
global `property/object`;
    ...
    elif type(expr,indexed) and member( expr, SCALELIST) then
	    res:=CONVERT2SERIES('SERIES'(scale,[1],0,'integer',[1],infinity,
		'integer',expr,expr),scale,var,true)
	# it is necessary to add "not has(expr,O)" for series(f(x)+O(RootOf(_Z^2+1)),x) 
	# it is necessary to add (rational) for 
	# 	limit((x^2+5*x+7)/(x^3+5),x = RootOf(_Z^3+5,-1.709975947),real)
	# it is necessary to do it only when var=[SCALEVARIABLE] so that the result
	# is in the right scale
    elif type(expr,ratpoly(rational)) and var=[SCALEVARIABLE]
		and select(has,indets(expr),_var) minus {SCALEVARIABLE}={} then
        try
    	    res:=oldseries(expr,var[-1],max(ord,3));
    	catch "division by zero series":
    	    # this happens with series(1/(x^3-x^4),x,3)
    	    return procname(expr,scale,var,2*max(ord,3))
    	end try;
        if res=0 then return 0 fi;
        # Handle the difference of meaning of Order between oldseries and series
        if op(1,res)=O(1) then 
                Order:=max(ord,3);
	        to 5 do
	                Order:=2*Order;
	                res:=oldseries(expr,var[-1]);
	                if op(1,res)<>O(1) then break fi
	        od
	fi;
	if op(1,res)=O(1) then
	        newexpr:=normal(expr);
	        Order:=max(ord,3);
	        while op(1,res)=O(1) do
	                Order:=2*Order;
	                res:=oldseries(newexpr,var[-1])
	        od
	fi;
        res:=series2SERIES(res,scale,var,ord);
        if res<>0 then res:=subsop(EXPR4SERIES=expr,res) fi
    elif type(expr,`+`)
	...
	# should not go through run.
        if op(0,expr)=O then # special case for O() must be done before
...
#		    subs(scale['variable']=vname,scale['back']),[op(expr)]);
		...
		elif [op(0..-1,res)]=[fcn,op(largs)] or 
		    type(res,':-series') and op([1,0..1],res)=[fcn,op(largs)] then
...
local L, ser2, argsf, ser1, sc0, v, i, nbargs, vars;
    ...
    od;
    ## Create a scale for intermediate computations
    vars:=[seq(v[i],i=1..nbargs)];
    sc0:=map(newscale,vars,0);
    ## Perform the computation recursively on each argument
    `multiseries/D/recurse`(f(seq(subs(v[i]=eval(sc0[i]['variable'],2),argsf[i]),i=1..nbargs)),
...
local res, dorder, narg, nbdiff;
    ...
    if ser[-1]=0 then res:=CONVERT2SERIES(subs(v[-1]=0,f),scale,var,true)
    else
