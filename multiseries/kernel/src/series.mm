...
SERIES := proc(	scale       :: SCALE	 , lcoeffs   :: list	 ,                coeffO      :: algebraic , typeCoeff :: type	 ,
...
## This is too early: in intermediate computations expr4SERIES is not
## maintained.
#    if expr4SERIES = 0 then 0
#    else
	# suppress zero terms
	if member(0,lcoeffs) or member(0.,lcoeffs) then
	    for i to nops(lcoeffs) do ind[i]:=NULL od;
	    if member(0,lcoeffs) then
	    	for i to nops(lcoeffs) do
	    	    if lcoeffs[i]=0 then ind[i]:=i=NULL fi
	    	od
	    fi;
	    if member(0.,lcoeffs) then
	    	for i to nops(lcoeffs) do
	    ...
	    ind:=seq(ind[i],i=1..nops(lcoeffs));
	    expons:=subsop(ind,lexpons);
	    coeffs:=subsop(ind,lcoeffs)
	else
	    coeffs:=lcoeffs;
	    expons:=lexpons
	fi;
	...
	if coeffs<>[] and typeCoeff='t_SERIES' then
        ...
        # Output 
	if coeffs=[] and (coeffO=0 and exponO=infinity or expr4SERIES=0) then 0
	else
