...
local res :: {SERIES,identical(0)}, bigo, scale1, genpt, actpt, fcn, genser1, v, rest, first, v0, sc0;
    ...
    elif op(EXPR4SERIES,ser2)=SCALEVARIABLE and eval(scale,1)=eval(scale1,1) and [op(EXPANVAR,ser1)]=var then
	...
	if not has(actpt,SCALEVARNAME) then # hidden 0!
	    return procname(ser1,
	    	subsop(LISTCOEFF=subsop(1=NULL,op(LISTCOEFF,ser2)),
	    	       LISTEXPON=subsop(1=NULL,op(LISTEXPON,ser2)),ser2),
	    	       args[3..-1])
	fi;
	...
	if rest=0 then res:=first
	    ...
	    sc0:=newscale(v0,0);
	    # The point is generic, it is not on a branch-cut
	    _EnvBranchCut:=false;
	    genser1:=RUN(subs(v=genpt+sc0['variable'],fcn)-subs(v=genpt,fcn),sc0,
	    ...
	    # recognized:
	    if genser1<>0 and 
	    	op(LISTEXPON,genser1)<>[] and op([LISTEXPON,1],genser1)=0 then
		genser1:=subsop([LISTCOEFF,1]=NULL,[LISTEXPON,1]=NULL,genser1)
	    fi;
	    # Special case when fcn was a constant in disguise that RUN
	    # detected so that genser1 is 0
	    if genser1=0 then res:=first
	    else
		res:=ADDDOIT(first,
#		    procname(subs(genpt=actpt,genser1),rest,args[3..-1]))
		# this has to be eval because diff could be in the coeffs.
		procname(eval(genser1,genpt=actpt),rest,args[3..-1]))
	    fi
	fi
    elif op(LISTEXPON,ser2)<>[] and not
	COMPARISONFUNCTION(op(TYPEEXPON,ser2))(0,op([LISTEXPON,1],ser2)) then
	# remove initial terms that represent 0
	return procname(ser1,subsop(LISTCOEFF=subsop(1=NULL,op(LISTCOEFF,ser2)),
			     LISTEXPON=subsop(1=NULL,op(LISTEXPON,ser2)),ser2),
			     args[3..-1])
    else
	if op(LISTEXPON,ser1)=[] then res:=0
	else res := `multiseries/ComposeClassical`(ser1,ser2,args[3..-1])
	fi;
...
#    elif op(LISTCOEFF,ser)<>[] then
	...
	bigo:=op([LISTCOEFF,1],ser);
	if op(TYPECOEFF,ser)='t_SERIES' then	
	    bigo:=procname(bigo,scale);
#	    if not has(op(EXPR4SERIES,bigo),SCALELIST) then bigo:=1 fi;
	fi;
	subsop(LISTCOEFF=[],LISTEXPON=[],
	    EXPONBIGO=op([LISTEXPON,1],ser),
	    `if`(bigo=1,TYPECOEFF=integer,NULL),
	    COEFFBIGO=SERIES2BIGO(bigo,scale),ser)
    else ser
    fi;
...
option ALGOCOPYRIGHT;
local expon, res, pow, i, lcoeffs, valbigo, comp, bigo, firstexp, Ores, lser2, prevexp, setdiff;
	...
	if expon[1]<>0 then firstexp:=expon[1]
	    ...
	    return subsop(LISTCOEFF=lcoeffs,LISTEXPON=[0],COEFFBIGO=0,
	    	EXPONBIGO=infinity,
	    	EXPR4SERIES=op(EXPR4SERIES,ser1),ser2)
	fi;
    ...
    res:=0; Ores:=0; pow:=1; prevexp:=-infinity; lser2[1]:=ser2;
    
    setdiff:={seq(expon[i+1]-expon[i],i=1..nops(expon)-1)};
    if nops(setdiff)<3 then
    	for i in setdiff do if i<>1 then lser2[i]:=POWER(lser2[1],i,args[3..-1]) fi od
    else setdiff:={1}
    fi;
    for i to nops(expon) do
	if member(expon[i]-prevexp,setdiff) then pow:=MULDOIT(pow,lser2[expon[i]-prevexp])
	else pow:=POWER(lser2[1],expon[i],args[3..-1]) fi;
	pow,bigo:=TRUNCATE(pow,valbigo,comp);
	if expon[i]=firstexp then # Happens only once 
	    if op(LISTEXPON,ser2)=[] then lser2[1]:=0
	    else lser2[1]:=subsop(COEFFBIGO=0,EXPONBIGO=infinity,ser2)
	    fi
	fi;
