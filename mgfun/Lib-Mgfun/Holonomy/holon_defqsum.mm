...
holon_defqsum:=proc(poly_list::{list,set}(polynom),
    ...
    # Here, the envisioned type is qshift.  This type however is
    # too weak and badly-designed to allow for robust code.
    if {seq(Alg["type_of_left",x],x=x_list)}<>{'qdilat'} then
        error cat(x_list," not all associated to q-dilation operators")
    end if;
    takayama_algorithm(
